...
本期支持普通矩形检测框的跨工具交换和 COCO 实例分割，不覆盖所有任务与格式变体。
...
| `coco_segmentation` | COCO 实例分割的多边形与压缩/未压缩 RLE | 保留孔洞与分离区域；复杂多边形导入为掩码；按划分另附 JSON；RLE 图像上限 2³²−1 像素 |
...
4. 点击“检查导出兼容性”，核对图像/类别/对象数量与各格式的保留、转换、丢失项。
